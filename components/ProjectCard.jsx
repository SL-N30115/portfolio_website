...
const ProjectCard = ({ project }) => {
    return (
        <Card className='group overflow-hidden relative h-[430px]' >
            <CardHeader className='p-0'>
                ...
                <div className="relative w-full h-[300px] flex items-center justify-center bg-tertiary dark:bg-secondary/40 xl:bg-work_project_bg_light xl:bg-[110%] xl:dark:bg-work_project_bg_dark xl:bg-no-repeat overflow-hidden">
                    <Image className="absolute bottom-10 shadow-2xl" src={project.image} width={247} height={250} alt="" priority />
                    <div className="flex gap-x-4">
                        {/* buttons */}
                        {project.link != '/' && <Link rel="noopener noreferrer" target="_blank" href={project.link}
                            className="bg-secondary w-[54px] h-[54px] rounded-full flex justify-center items-center scale-0 opacity-0 group-hover:scale-100 group-hover:opacity-100 transition-all duration-200">
                            <Link2Icon className="" />
                        </Link>}
                        {project.github != '/' && <Link rel="noopener noreferrer" target="_blank" href={project.github}
                            className="bg-secondary w-[54px] h-[54px] rounded-full flex justify-center items-center scale-0 opacity-0 group-hover:scale-100 group-hover:opacity-100 transition-all duration-200">
                            <Github className="" />
                        </Link>}
                    </div>
